...
export const createFlashCardsJSON = async (
  ...
  try {
    ...
    const mergedPrompt = `
        ...
        Now, create flashcards in pure JSON format ONLY:
        [
    ...
    if (!rawOutput) {
      console.warn("No flashcards returned:", JSON.stringify(response.data, null, 2));
      return [];
    }
    ...
    try {
      ...
      console.error("Failed to parse flashcards JSON. Raw output:", rawOutput);
      return [];
    }
